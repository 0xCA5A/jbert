...
public abstract class DebouncedGpiAction implements GpioPinListenerDigital {
    ...
    private void logPinState(GpioPinDigitalStateChangeEvent event) {
        logger.debug("GPI pin state change ch.jbert.event received: {} = {} ({})", event.getPin(), event.getState(), event.getEdge());
    }
    ...
    private boolean isEventConsumed() {
        ...
        logger.trace("Ignore '{}' ch.jbert.event for {} more ms", eventTriggerEdge, millisDelta);
        return false;
    }
    ...
    public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent event) {
        ...
        if (event.getEdge() == eventTriggerEdge && isEventConsumed()) {
            logger.debug("Call registered action callback on valid pin '{}' ch.jbert.event", event.getPin());
